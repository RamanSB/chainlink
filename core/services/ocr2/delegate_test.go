...
func TestGetEVMEffectiveTransmitterID(t *testing.T) {
	...
	setTestCase := func(jb *job.Job, tc testCase, txManager *txmmocks.MockEvmTxManager) {
		...
		if tc.forwardingEnabled && tc.getForwarderForEOAErr {
			txManager.Mock.On("GetForwarderForEOA", mock.Anything, tc.getForwarderForEOAArg).Return(common.HexToAddress("0x0"), errors.New("random error")).Once()
		} else if tc.forwardingEnabled {
			txManager.Mock.On("GetForwarderForEOA", mock.Anything, tc.getForwarderForEOAArg).Return(common.HexToAddress(tc.expectedTransmitterID), nil).Once()
		}
	}
